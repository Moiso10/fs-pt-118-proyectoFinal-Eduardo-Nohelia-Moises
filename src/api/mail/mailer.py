...
def send_email(address, token):
    try:
        msg = Message(subject="Reset your password",  # Asunto del correo
        ...
        if os.getenv("FLASK_DEBUG") == "1":
            msg.html = f'''<a href="{os.getenv("FRONT_END")}/reset?token={token}">Hola, sigue este vinculo para resetear tu contraseña</a>'''
        else:
